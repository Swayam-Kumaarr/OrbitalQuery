...
export default function AnalysisSummary({ result }: AnalysisSummaryProps) {
  ...
  const isFallback = (result as any)?.metrics?.fallbackMode === true;
      ...
      <details className="oq-card overflow-hidden group">
        <summary className="px-4 py-2.5 text-[10px] font-semibold text-oq-200 uppercase tracking-wider cursor-pointer hover:text-oq-50 transition-colors flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            ...
            {isFallback ? 'Data Sources (Limited)' : 'Data Sources & Method'}
          </span>
        ...
        <div className="px-4 pb-3 space-y-3">
          {isFallback && confidence && (
            <div className="p-2.5 rounded bg-amber-500/10 border border-amber-500/20">
              <p className="text-[10px] text-amber-300 font-medium mb-0.5">Analysis Engine Offline</p>
              <p className="text-[11px] text-oq-200 leading-relaxed">Results are dataset matches from local database — no raster processing was performed.</p>
            </div>
          )}
          {!isFallback && confidence && <p className="text-[11px] text-oq-200 leading-relaxed">{confidence}</p>}
